...
const phases = ["Veg", "Early", "Mid", "Late", "Flush"]; const systems = [ "GH Maxi Series (Powder)", "GH 3-Part + KoolBloom (Liquid)", "MasterBlend 3-Part (Powder)", "Jack's 3-2-1 (Powder)", ];
...
const parseEC = (value) => { const num = parseFloat(value); if (isNaN(num)) return 0; if (num >= 100) return num / 500; // assume 500 scale ppm return num; };
...
if (system.includes("Maxi")) {
...
} else if (system.includes("Jack's")) {
  ...
  output = `Jack's Part A: ${(jack * gallons).toFixed(2)}g total for ${gallons} gal\n`;
  output += `Calcium Nitrate: ${(cal * gallons).toFixed(2)}g total\n`;
  output += `Epsom Salt: ${(epsom * gallons).toFixed(2)}g total`;
  ...
  {system && result && (
    <div
      className={`inline-block px-3 py-1 rounded-full text-sm font-semibold mb-2 ${
        ...
        system.includes("Jack's") ? "bg-yellow-100 text-yellow-800" :
