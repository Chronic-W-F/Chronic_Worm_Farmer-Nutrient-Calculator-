...
export default function Home() {
  ...
  const handleCalculate = () => {
    ...
    if (system.includes("Maxi")) {
      ...
      output = `${label}: ${(gramsPerGal * gallons).toFixed(
        2
      ...
      cal *= ecMultiplier;
      epsom *= ecMultiplier;
      output = `MasterBlend: ${(mb * gallons).toFixed(
        2
      )}g total\nCalcium Nitrate: ${(cal * gallons).toFixed(
        2
      ...
      cal *= ecMultiplier;
      epsom *= ecMultiplier;
      output = `Jacks Part A: ${(jack * gallons).toFixed(
        2
      )}g\nCalcium Nitrate: ${(cal * gallons).toFixed(
        2
      )}g\nEpsom Salt: ${(epsom * gallons).toFixed(2)}g`;
