...
const phases = ["Veg", "Early", "Mid", "Late", "Flush"];
const systems = [
  "GH Maxi Series (Powder)",
  "GH 3-Part + KoolBloom (Liquid)",
  "MasterBlend 3-Part (Powder)",
  "Jacks 3-2-1 (Powder)",
];
...
export default function Home() {
  ...
  const [selectedPhase, setSelectedPhase] = useState("Veg");
  const [system, setSystem] = useState(systems[0]);
  const [result, setResult] = useState("");
  const [flushPhrase, setFlushPhrase] = useState("");

  const flushPhrases = [
    "Hydrated. High. Accomplished.",
    "No nutes. Just vibes.",
    "Time to coast to the finish line.",
    "This plant's been through enough.",
    "Flush it like your browser history.",
    "Clean roots, clean conscience.",
    "Nothing left but trichomes and dreams.",
    "If you’re reading this, you’re done feeding.",
    "All gas, no nutrients.",
    "Nute withdrawals initiated.",
    "Don’t be salty—flush it out.",
    "Riding the fade like a pro.",
    "Time to let go, she’s ready.",
    "No powder. No liquid. Only peace.",
    "Let her starve. She deserves it.",
    "Straight water diet begins now.",
    "Bye bye ppm.",
  ...
  const parseEC = (value) => {
    ...
    if (num >= 100) return num / 500; // assume 500 scale ppm
    return num;
  };
  ...
  return (
    <div className="p-6 max-w-xl mx-auto">
      ...
      {/* Color-coded nutrient system label */}
      {system && result && (
        <div
          className={`inline-block px-3 py-1 rounded-full text-sm font-semibold mb-2 ${
            system.includes("Maxi") ? "bg-blue-100 text-blue-800" :
            system.includes("MasterBlend") ? "bg-green-100 text-green-800" :
            system.includes("Jacks") ? "bg-yellow-100 text-yellow-800" :
            system.includes("GH 3-Part") ? "bg-purple-100 text-purple-800" :
